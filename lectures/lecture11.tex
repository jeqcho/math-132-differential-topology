...
\begin{corollary}{Transversal Maps are Generic (Euclidean Target)}{transversal-generic-euclidean}
    Let $M$ be a smooth manifold with boundary, $f \colon M \to \R^\ell$ a smooth map, and $P \subset \R^\ell$ a submanifold. Then for almost every $s \in \R^\ell$, the translated map $f_s(x) := f(x) + s$ satisfies $f_s \pitchfork P$ and $f_s|_{\partial M} \pitchfork P$.

    In particular, $s$ may be chosen arbitrarily small.
\end{corollary}

\begin{proof}
    Define $F \colon M \times \R^\ell \to \R^\ell$ by $F(x, s) = f(x) + s$. For any fixed $x \in M$, the map $s \mapsto F(x, s)$ is a translation, hence a submersion. So $F$ itself is a submersion, as is $F|_{\partial M \times \R^\ell}$. In particular, both are transversal to any submanifold $P \subset \R^\ell$.

    The Transversality Theorem then gives $f_s \pitchfork P$ and $f_s|_{\partial M} \pitchfork P$ for almost every $s \in \R^\ell$. Since the exceptional set has measure zero, it has empty interior, so $s$ may be chosen arbitrarily small.
\end{proof}
